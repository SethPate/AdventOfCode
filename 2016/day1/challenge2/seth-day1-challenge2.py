	...
	if bearing == "north":
		if index[0] == "R":
			...
			while xdif != 0:
				...
				finish[0] = finish[0] + 1 #updates our coordinates
				xdif = xdif - 1
			print (finish)
			bearing = "east"
		else:
		...
		if index[0] == "R":
			...
			while ydif != 0:
				locations.append([finish[0], int(finish[1]) + 1])
				finish[1] = finish[1] + 1 #updates our current coordinates
				ydif = ydif - 1 #counts down the difference
			print (finish)
			bearing = "north"
		else:
			print ("turns south")
...
for i in locations:
	if locations.count(i) > 1:
		print (i, "Distance from finish: " + str(abs(i[0] - finish[0]) + abs(i[1] - finish[1])) + " steps") #how far am I from the overlap point?
		break