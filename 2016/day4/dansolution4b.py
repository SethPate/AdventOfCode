...
def decrypter(inputstring,shift):
	finalstring = ""
	inputlength = len(inputstring)
	letterset = "abcdefghijklmnopqrstuvwxyz"
	for everyletter in range(0,inputlength):
		g = letterset.find(inputstring[everyletter])
		shiftedletter = g + shift
		h = shiftedletter % 26
		finalstring = str(finalstring) + str(letterset[h])
	return finalstring
...
for k in range(0,setlength):
	...
	if decrypter(letterstring,sectorcode).find('north') >= 0:
		print str(sectorcode) + " " + decrypter(letterstring,sectorcode)
